...
def likelihood_plot(x, y1, y2, boundary, x_lim=[-5, 5], y_lim=[-0.02, 0.5]):
    ...
    for b in boundary:
        plt.axvline(x=b, linewidth=2, color='black', alpha=0.75, label=f'boundary: x={b}')
...
def double_threshold_boundary(mu1, mu2, sigma1, sigma2):
    ...
    root = math.sqrt((mu1/sigma1**2 - mu2/sigma2**2)**2 - 4*(1/(2*sigma2**2) - 1/(2*(sigma1**2)))*(-mu1**2/(2*(sigma1**2)) + mu2**2/(2*(sigma2**2)) - np.log(sigma1) + np.log(sigma2)))

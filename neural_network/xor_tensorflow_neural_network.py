...
class XORNN:
    def __init__(
        self, max_iteration, learning_rate, cost_function_type, hidden_node_act
    ):
    ...
    def learning_step(self):
        return tf.train.GradientDescentOptimizer(self.learning_rate).minimize(
            self.cost_function
        )
    ...
    def start_learning(self):
        ...
        for i in range(self.max_iteration):
            error = sess.run(learning_algor, feed_dict={self.x_: XOR_X, self.y_: XOR_Y})
            cost = float(
                sess.run(self.cost_function, feed_dict={self.x_: XOR_X, self.y_: XOR_Y})
            )
            self.costs.append(round(cost, 4))
        ...
        predicted = sess.run(self.layer2, feed_dict={self.layer1: l1, self.y_: XOR_Y})
